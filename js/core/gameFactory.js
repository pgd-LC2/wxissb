...
(() => {
  ...
  function makeGame() {
    ...
    if (GameApp.Progression && GameApp.Progression.attachCombatRating) {
      ...
      g.stats = { startTime: 0, kills: 0, dmgDealt: 0, dmgTaken: 0, expGained: 0 };
      g.combat = { rating: 0, ratingSmooth: 0, tier: "D", tierColor: "rgba(255,255,255,.92)", peak: 0, integral: 0, _acc: { kills: 0, dmgDealt: 0, dmgTaken: 0, exp: 0, levelUps: 0 }, events: { kills: [], dmgDealt: [], dmgTaken: [], exp: [], levelUps: [] } };
    }
  ...
  g.showDodgeEffect = (t) => {
    ...
    if (GameApp.Progression && GameApp.Progression.attachSkillSelection) {
      GameApp.Progression.attachSkillSelection(g, { safeNonNeg });
    } else {
      g.generateSkills = () => { g.skillChoices = []; };
      g.selectSkill = () => {};
      g.resetGame = () => {};
    }
    ...
    if (GameApp.RenderSystems && GameApp.RenderSystems.attachRenderer) {
      GameApp.RenderSystems.attachRenderer(g);
    } else {
      g.render = () => {};
    }
