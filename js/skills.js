...
function generateAllSkills(baseBladeSkills = []) {
  ...
  // 基础属性类 (原有)
  // ------------------------------
  skills.push({ name:"强力子弹", description:"伤害 +12%", tier:1, icon:"bolt.fill", effect:(s)=>{ s.bulletDamage *= 1.12; }});
  skills.push({ name:"极速射击", description:"射速 +10%", tier:1, icon:"timer", effect:(s)=>{ s.shootInterval *= 0.90; }});
  ...
  skills.push({ name:"生命强化", description:"最大生命 +25", tier:1, icon:"heart.fill", effect:(s)=>{ s.playerMaxHealth += 25; s.playerHealth += 25; s.updateHealthUI(); }});
  ...
  skills.push({ name:"精准射手", description:"暴击率 +6%", tier:1, icon:"target", effect:(s)=>{ s.critRate += 0.06; }});
  ...
  // 子弹变体类 (原有)
  // ------------------------------
  skills.push({ name:"多重射击", description:"子弹数量 +1", tier:3, icon:"arrow.up.forward.and.arrow.down.backward", effect:(s)=>{ s.bulletCount = Math.min((s.bulletCount || 1) + 1, 8); }});
  skills.push({ name:"散弹枪", description:"+2 子弹，散布角度增大", tier:3, icon:"arrow.branch", effect:(s)=>{ s.bulletCount += 2; s.spreadAngle += 0.20; }});
  ...
  skills.push({ name:"强力追踪", description:"增强子弹追踪能力", tier:3, icon:"location.north.line.fill", effect:(s)=>{ s.homingStrength += 0.35; }});
  skills.push({ name:"冰冻弹", description:"子弹有 15% 几率冰冻敌人 0.8秒", tier:3, icon:"snowflake", effect:(s)=>{ s.freezeChance += 0.15; s.freezeDuration = Math.max(s.freezeDuration, 0.8); }});
  skills.push({ name:"寒冰大师", description:"冰冻几率 +20%，持续 +0.6秒", tier:4, icon:"snowflake.circle.fill", effect:(s)=>{ s.freezeChance += 0.2; s.freezeDuration += 0.6; }});
  skills.push({ name:"燃烧弹", description:"子弹有 40% 几率点燃敌人，2.5秒内造成额外伤害", tier:3, icon:"flame.fill", effect:(s)=>{ s.burnChance = Math.max(s.burnChance || 0, 0.4); s.burnDamage = Math.max(s.burnDamage || 0, 4); s.burnDuration = Math.max(s.burnDuration || 0, 2.5); }});
  ...
  skills.push({ name:"毒弹", description:"子弹有 45% 几率使敌人中毒，持续掉血 4秒", tier:3, icon:"leaf.fill", effect:(s)=>{ s.poisonChance = Math.max(s.poisonChance || 0, 0.45); s.poisonDamage = Math.max(s.poisonDamage || 0, 3); s.poisonDuration = Math.max(s.poisonDuration || 0, 4.0); }});
  ...
  skills.push({ name:"超导闪电", description:"闪电跳跃 +1 次，伤害衰减减轻", tier:5, icon:"bolt.circle.fill", effect:(s)=>{ s.chainCount += 1; s.chainDamageDecay = Math.max(s.chainDamageDecay || 0.7, 0.85); }});
  ...
  skills.push({ name:"护盾矩阵", description:"护盾球 +1，旋转速度提升", tier:4, icon:"shield.checkered", effect:(s)=>{ s.orbitalShieldCount += 1; s.orbitalShieldSpeed *= 1.3; }});
  ...
  skills.push({ name:"闪避大师", description:"12% 几率完全闪避伤害", tier:3, icon:"figure.dodge", effect:(s)=>{ s.dodgeChance += 0.12; }});
  skills.push({ name:"幻影", description:"闪避成功后短暂无敌", tier:4, icon:"figure.2.arms.open", effect:(s)=>{ s.dodgeInvincibility = true; }});
  skills.push({ name:"无敌帧强化", description:"受伤后无敌时间 +0.25秒", tier:2, icon:"clock.badge.checkmark", effect:(s)=>{ s.iFrameDuration += 0.25; }});
  ...
  skills.push({ name:"血之契约", description:"吸血几率提升至 35%，但最大生命 -20%", tier:4, icon:"drop.triangle.fill", effect:(s)=>{ s.lifestealChance = Math.max(s.lifestealChance || 0, 0.35); s.lifestealPercent = Math.max(s.lifestealPercent || 0, 0.12); s.playerMaxHealth *= 0.8; s.playerHealth = Math.min(s.playerHealth, s.playerMaxHealth); }});
  ...
  skills.push({ name:"不死鸟", description:"死亡时有 30% 几率复活并回复 30% 血量", tier:5, icon:"bird.fill", effect:(s)=>{ s.phoenixRevive = true; s.phoenixChance = 0.3; }});
  skills.push({ name:"坚韧", description:"单次伤害上限为最大生命的 40%", tier:5, icon:"figure.strengthtraining.traditional", effect:(s)=>{ s.damageCap = 0.4; }});
  ...
  skills.push({ name:"无人机编队", description:"无人机 +1，攻击力提升", tier:4, icon:"airplane.circle.fill", effect:(s)=>{ s.droneCount += 1; s.droneDamage *= 1.35; }});
  ...
  skills.push({ name:"死亡之舞", description:"刀片数量 +2，范围扩大", tier:4, icon:"fan.and.light.ceiling.fill", effect:(s)=>{ s.bladeOrbitCount += 2; s.bladeOrbitRadius *= 1.25; }});
  ...
  skills.push({ name:"流星雨", description:"陨石数量 +1，间隔缩短 20%", tier:4, icon:"sparkles", effect:(s)=>{ s.meteorCount += 1; s.meteorInterval *= 0.8; }});
  ...
  skills.push({ name:"杀戮狂欢", description:"连杀奖励上限提高，衰减略减慢", tier:4, icon:"flame.circle.fill", effect:(s)=>{ s.killStreakMaxBonus *= 1.3; s.killStreakDecay *= 0.8; }});
  ...
  skills.push({ name:"幸运一击", description:"4% 几率造成 5倍 伤害", tier:4, icon:"star.circle.fill", effect:(s)=>{ s.luckyCritChance = 0.04; s.luckyCritMulti = 5.0; }});
  skills.push({ name:"欧皇附体", description:"幸运一击几率额外 +2%", tier:5, icon:"crown.fill", effect:(s)=>{ s.luckyCritChance += 0.02; }});
  skills.push({ name:"处决", description:"对低血量敌人造成额外伤害", tier:3, icon:"scissors", effect:(s)=>{ s.executeEnabled = true; s.executeThreshold = 0.3; }});
  skills.push({ name:"斩杀", description:"直接击杀 10% 血量以下的敌人", tier:4, icon:"scissors.badge.ellipsis", effect:(s)=>{ s.instantKillThreshold = 0.1; }});
  skills.push({ name:"超载", description:"暴击时有 18% 几率再次攻击", tier:3, icon:"bolt.badge.a.fill", effect:(s)=>{ s.overloadChance = 0.18; }});
  ...
  skills.push({ name:"弹幕", description:"同时向多个方向射击", tier:5, icon:"circle.and.line.horizontal.fill", effect:(s)=>{ s.bulletCount += 4; s.spreadAngle = TAU / Math.max(1, s.bulletCount); s.allDirectionFire = true; }});
  ...
  skills.push({ name:"玻璃大炮", description:"伤害 +60%，但生命值 -35%", tier:5, icon:"sparkle.magnifyingglass", effect:(s)=>{ s.bulletDamage *= 1.6; s.playerMaxHealth *= 0.65; s.playerHealth = Math.min(s.playerHealth, s.playerMaxHealth); }});
  ...
  skills.push({ name:"共生", description:"附近敌人越多伤害越高（上限30%）", tier:3, icon:"person.3.sequence.fill", effect:(s)=>{ s.symbiosisEnabled = true; }});
  skills.push({ name:"清场", description:"屏幕内敌人较少时，伤害提升", tier:3, icon:"rectangle.badge.minus", effect:(s)=>{ s.clearingBonus = true; }});
